...
export OneMil_CitiesByStateSearch() := macro
import std;
...
ds:=dataset([],rec);
output(ds);  //gives the output of an empty table without values
df:='~krishna::key::stcitycounty::prod::payload::mil';
...
output(sort(if(unique_val,dedup(final_out,record),final_out),city,county), named ('Results'));
